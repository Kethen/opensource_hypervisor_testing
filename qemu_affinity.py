	...
	for (arg, thread_name_re, thread_name_fmt) in classes:
		...
		if len(affinities) != 0:
			...
			if isinstance(selector, PositionalThreadSelector):
				print(arg, thread_name_re, thread_name_fmt)
				parser.error('argument %s: too many positional affinity specifications (%d threads, %d provided).' % (argparse._get_action_name(arg), len(list(threads)), spec_count))
